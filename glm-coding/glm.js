...
(function () {
  ...
  function startWatching() {
    ...
    const ts = `${config.targetHour}:${String(config.targetMinute).padStart(2, '0')}:${String(configSecond || 0).padStart(2, '0')}`;
    log(`开始闭环监听，目标时间: ${ts}`);
    updateStatus(getIdleStatusText());
    scheduleNextTick(0);
  }
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .glm-simple-status-v16{font-size:13px;margin-bottom:10px;padding:8px;background:#f1f5f9;border-radius:8px;text-align:center;font-weight:bold;color:#140af;}
