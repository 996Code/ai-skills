...
> 本文件是项目级指令的**单一真相源**。`CLAUDE.md`（Claude Code 用）与 `AGENTS.md`（opencode 用）都是指向本文件的软链接——**改这里，两个工具同步生效**。LLM/provider 配置由用户全局管理，本项目不含。
...
命令定义位置：Claude Code 在 `.claude/commands/`，opencode 在 `.opencode/commands/`。两者内容一致，都是"去读 `skill/SKILL.md` 对应阶段"。
...
- **opencode**：`.opencode/plugins/novel-check.ts`（写前门禁 `throw` 阻断 + 写后日志）
